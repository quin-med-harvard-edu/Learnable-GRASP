...
class RacerGRASP:
    ...
    def _objective_pre_comp(self, x, X, Y):
        ...
        if multi_flag:
            # Calculate ||X-y||_2
            tmp_ = X - Y
            tmp_ *= np.transpose(self.weight_coil.reshape((-1,) + (1,) * 3),
                                 (1, 2, 0, 3))
    ...
    def _derivative_pre_comp(self, x, xX, y):
        ...
        if multi_flag:
            ...
            del_l2 *= np.transpose(self.weight_coil.reshape((-1,) + (1,) * 3),
                                   (1, 0, 2, 3))
            del_l2 = np.sum(del_l2, axis=1)
            # Calculate del(||TV(x)||_2)/d(x)|_x
            if self.lam:
                tmp_ = x[1:, :, :] - x[:-1, :, :]
