...
from motion_correction.registration.base_registration import RegistrationObject
...
class MotionGRASP:
    ...
    def __init__(self,
                 ...
                 lam: float = None,
                 register_obj=RegistrationObject,
                 register_grp=list):
        ...
        self.register_grp = register_grp
    ...
    def reconstruct(self,
        ...
        return x_rec, reg_map, reg_i_map
    ...
    def _objective_pre_comp(self, x, X, Y, reg_map=None):
        ...
        if multi_flag:
            ...
            if self.lam:
                if reg_map:
                    x = self.register_obj.register_with_map(x, reg_map)
                tmp_ = x[1:, :, :] - x[:-1, :, :]
        ...
        if multi_flag:
            ...
            if self.lam:
                ...
                del_l1 = np.concatenate(([-tmp_[0, :, :]],
                                         ...
                                         [tmp_[-1, :, :]]), axis=0)
                if reg_map:
                    del_l1 = self.register_obj.register_with_map(del_l1, reg_i_map)
            else:
                del_l1 = 0
        else:
            raise (NotImplementedError)
    ...
    def _compute_registration_map(self, xk_hat):
        ...
        _xk = []
        for idx_grp in list(np.unique(self.register_grp)):
            list_idx = np.where(self.register_grp == idx_grp)[0]
            if len(list_idx) > 1:
                _xk.append(np.mean(np.squeeze(xk_hat[list_idx, :, :]), axis=0))
            else:
                _xk.append(np.squeeze(xk_hat[list_idx, :, :]))
        _xk = np.array(_xk)

        # idx_ref = int(np.bincount(self.register_grp).argmax()) - 1
        idx_ref = _xk.shape[0] - 1

        list_map, list_i_map = self.register_obj.get_maps(xk=_xk, idx_ref=idx_ref)

        tmp_ = [copy(list_map[idx]) for idx in list(self.register_grp)]
        list_map = copy(tmp_)
        del tmp_
        tmp_ = [copy(list_i_map[idx]) for idx in list(self.register_grp)]
        list_i_map = copy(tmp_)
        del tmp_
